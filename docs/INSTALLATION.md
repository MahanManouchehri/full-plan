...
```powershell
git -C "C:\Users\<your-user>\.codex\skills\api-feature-plan" pull --ff-only
```
...
فقط نصب Skill، الزام استفاده از آن را در یک پروژه ایجاد نمی‌کند. برای فعال‌سازی خودکار در پروژه، در `AGENTS.md` بنویسید که پیش از طراحی یا پیاده‌سازی فیچر APIمحور، `api-feature-plan` اجرا شود و تا تأیید کاربر، کدنویسی آغاز نشود.

اگر Skill در مسیر سراسری نبود، می‌توانید در همان دستورالعمل، مخزن رسمی این پروژه را به‌عنوان منبع بازیابی معرفی کنید:

```text
https://github.com/MahanManouchehri/api-feature-plan
```
